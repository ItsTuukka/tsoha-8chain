...
def get_list():
    ...
    result = db.session.execute(sql, {"chain_id":chains.chain_id()})
...
def send(content):
    ...
    db.session.execute(sql, {"content":content, "topic_id":topics.topic_id(), "chain_id":chains.chain_id(), "user_id":users.user_id()})
    db.session.commit()
    return True
